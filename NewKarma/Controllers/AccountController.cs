...
namespace NewKarma.Controllers
{
    public class AccountController : Controller
    {
        ...
        public AccountController(IApplicationUserManager userManager, IApplicationRoleManager roleManager, IHostingEnvironment env, SignInManager<ApplicationUser> signInManager, IEmailSender emailSender, AppDbContext context)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (User != null)
                {
                    if (User.Status)
                    {
                        ...
                        if (result.Succeeded)
                        {
                            var userAdmin = await _userManger.IsInRoleAsync(User, "ادمین");
                            var userAuthor = await _userManger.IsInRoleAsync(User, "نماینده");
                            var userNormal = await _userManger.IsInRoleAsync(User, "کاربر");
                            if (userAdmin)
                            {
        ...
        public async Task<IActionResult> ForgetPassword(ForgetPasswordViewModel ViewModel)
        {
            if (ModelState.IsValid)
            {
                var User = await _userManger.FindByEmailAsync(ViewModel.Email);
                if (User == null)
                {
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordViewModel ViewModel)
        {
            ...
            else
            {
                ...
                var Result = await _userManger.ResetPasswordAsync(User, ViewModel.Code, ViewModel.Password);
                if (Result.Succeeded)
                {
